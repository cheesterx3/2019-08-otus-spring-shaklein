...
<script>
    ...
    export default {
        ...
        mounted() {
            this.$store.dispatch('registry/getAllAuthors');
            this.$store.dispatch('registry/getAllGenres');
        },
        computed: {
            ...mapState({
                authors: state => state.registry.authors,
                genres: state => state.registry.genres,
            })
        },
    }
</script>
