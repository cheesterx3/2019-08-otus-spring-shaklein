package ru.otus.study.spring.libraryweb.changelogs;
...
public class InitMongoDBDataChangeLog {
    ...
    public void initBooks(MongoTemplate template){
        final Book warAnPeaceBook = new Book("Война и мир", Collections.singletonList(authors.get(0)), Collections.singletonList(genres.get(0)));
        warAnPeaceBook.getGenres().add(genres.get(1));
        books.add(template.save(warAnPeaceBook));
        final Book secondBook = new Book("На дне", Collections.singletonList(authors.get(2)), Collections.singletonList(genres.get(1)));
        books.add(template.save(secondBook));
        final Book thirdBook = new Book("Сборник рассказов и повестей", Collections.singletonList(authors.get(1)), Collections.singletonList(genres.get(2)));
